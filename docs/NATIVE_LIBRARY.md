...
Pick **option 4 -- Verify age 21+ (native library demo)** and enter a
birth date as three numbers. The program prints `OK`, `UNDER`,
`BAD INPUT`, or `LIB ERROR` based on the native library's result and the
wrapper's load state.
